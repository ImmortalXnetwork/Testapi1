...
export const scrapeAnimeDetails = async ({ id }) => {
 try {
  ...
  const status = $('div.anime_info_body_bg > p.type:contains("Status") a').text();

  const otherName = $('div.anime_info_body_bg > p.other-name > a')
   .map((index, element) => $(element).attr('title').trim())
   .get();

  $('div.anime_info_body_bg > p.type:contains("Genre") a')
    .map((index, element) => genres.push($(element)
    .text()
    .replace(/, /g, '')));
